...
def ritz_min_and_vector(num_qubit, ops, fixed_x, H_mat, U_mat, n=3):
    """
    ...
    """
    dev_eval = qml.device("lightning.qubit", wires=num_qubit)
...
def calculate_overlap(state_a, state_b):
    """
    |<a|b>|^2
    """
...
def run_n_sweep_for_candidates(top_candidates, ns, num_qubit, fixed_x, H_mat, U_mat, target_state, E0):
    ...
    for rank, cand in enumerate(top_candidates, start=1):
        ...
        for n in ns:
            ...
            sweep_ovK.append(float(calculate_overlap(phi, target_state)))
            sweep_gap.append(float(calculate_energy_gap(E_ritz, E0)))
